...
export default function PrintLaporanPJU() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res = await fetch('http://localhost/DISHUP_S/dishup/api/pju.php');
            const json = await res.json();
            if (json.status === 'success') {
                setRows(json.data);
            }
        } catch (error) {
            console.error("Gagal load data PJU", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    // Dapatkan list kecamatan unik
    const kecamatans = useMemo(() => {
        const unique = [...new Set(rows.map((r) => r.kecamatan))].filter(Boolean);
        return ["Semua", ...unique];
    }, [rows]);

    // Filter baris
    const filteredRows = useMemo(() => {
        if (selectedKecamatan === "Semua") return rows;
        return rows.filter((r) => r.kecamatan === selectedKecamatan);
        ...
        <div className="p-4">
            ...
            <div className="print-area">
                ...
                <div className="text-center my-3">
                    <h5 className="fw-bold mb-1">LAPORAN DATA PENERANGAN JALAN UMUM (PJU)</h5>
                            ...
                            <tbody>
                                {filteredRows.map((r, i) => (
                                    <tr key={r.id}>
                                        ...
                                        <td>{r.kecamatan}</td>
                                        <td>{r.lokasi}</td>
                                        <td className="text-center">{r.lat}</td>
                                        <td className="text-center">{r.lng}</td>
                                        <td className="text-center fw-bold">{r.status.toUpperCase()}</td>
                                        <td className="text-center">{r.tahun}</td>
                                    </tr>
                                ))}
                                ...
                                {filteredRows.length === 0 && (
                                    <tr>
                                        <td colSpan={8} className="text-center text-muted py-4">
                                            Data PJU masih kosong atau gagal dimuat.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
