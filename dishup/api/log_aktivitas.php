...
if ($method === 'GET') {
    try {
        ...
        $stmtTotal = $pdo->query("SELECT
            (SELECT COUNT(*) FROM tbl_log_aktivitas WHERE DATE(created_at) = CURDATE() AND tipe_log = 'Login' AND status = 'Berhasil') as login_hari_ini,
            (SELECT COUNT(*) FROM tbl_log_aktivitas WHERE DATE(created_at) = CURDATE() AND tipe_log = 'Aktivitas') as aktivitas_hari_ini,
            (SELECT COUNT(*) FROM tbl_log_aktivitas WHERE DATE(created_at) = CURDATE() AND tipe_log = 'Perubahan Data') as perubahan_hari_ini,
            (SELECT COUNT(*) FROM tbl_log_aktivitas WHERE DATE(created_at) = CURDATE() AND tipe_log = 'Login' AND status = 'Gagal') as gagal_login_hari_ini
        ");
